...
import { handleOctoMessageAction, parseTarget, resolveOutboundOctoTarget, normalizeOutboundOctoChannelPrefix, extractInlineMentionUids } from "./actions.js";
...
export function resolveOutboundAccountId(ctxTo: string, fallbackAccountId: string): string {
  ...
  let targetForParse = normalizeOutboundOctoChannelPrefix(ctxTo);
...
export const octoPlugin: ChannelPlugin<ResolvedOctoAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (wsUrl.startsWith("ws://") && !wsUrl.startsWith("ws://localhost") && !wsUrl.startsWith("ws://127.")) {
        log?.warn?.(`[octo] WARNING: WebSocket URL "${wsUrl}" uses plain ws:// — bot token will be transmitted in cleartext. Use wss:// in production.`);
      }

...
export class WKSocket extends EventEmitter {
  ...
  private doConnect(): void {
    ...
    ws.on("error", (err) => {
      if (this.ws !== ws) return; // stale guard
      this.opts.log?.debug?.("[WKSocket] ws error:", err.message);
  ...
  private restartHeart(): void {
    ...
    this.heartTimer = setInterval(() => {
      ...
      if (this.pingRetryCount > this.pingMaxRetry) {
        this.opts.log?.debug?.("[WKSocket] ping timeout, reconnecting...");
        this.stopHeart();
        this.clearStableTimer();
        if (this.ws) {
          try { this.ws.close(); } catch { /* ignore */ }
  ...
  private handleRawData(data: Uint8Array): void {
    ...
    try {
      ...
      this.opts.log?.debug?.("[WKSocket] decode error:", err);
  ...
  private onRecv(dec: Decoder, _noPersist: boolean, _reddot: boolean): void {
    ...
    try {
      ...
      this.opts.log?.debug?.("[WKSocket] payload decrypt/parse error:", err);
      return;
    }

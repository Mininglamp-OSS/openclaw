...
export type OctoAccountConfig = OctoConfig & {
  accounts?: Record<string, OctoConfig | undefined>;
};
...
export function listOctoAccountIds(cfg: OpenClawConfig): string[] {
  const channel = getChannelConfig<OctoAccountConfig>(cfg);
...
export function resolveOctoAccount(params: {
  ...
  const channel = getChannelConfig<OctoAccountConfig>(params.cfg);

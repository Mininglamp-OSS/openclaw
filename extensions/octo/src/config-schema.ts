...
export const OctoConfigJsonSchema = {
  ...
  runtime: {
    safeParse(value: unknown): { success: boolean; data?: OctoConfig; error?: unknown } {
      ...
      if (v.accounts !== undefined && (typeof v.accounts !== "object" || Array.isArray(v.accounts))) {
        return { success: false, error: "accounts must be an object" };
      }
      if (v.pollIntervalMs !== undefined && typeof v.pollIntervalMs !== "number") {
        return { success: false, error: "pollIntervalMs must be a number" };
      }
      if (v.heartbeatIntervalMs !== undefined && typeof v.heartbeatIntervalMs !== "number") {
        return { success: false, error: "heartbeatIntervalMs must be a number" };
      }

...
export function broadcastGroupMdUpdate(params: {
  ...
  const { accountId, groupNo, content, version, log } = params;
  const meta: GroupMdMeta = {
    version,
  ...
  try {
    ...
    log?.info?.(`octo: broadcastGroupMdUpdate: updated disk cache group=${groupNo} v=${version}`);
  } catch (err) {
    log?.error?.(`octo: broadcastGroupMdUpdate failed for group=${groupNo}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
export function broadcastThreadMdUpdate(params: {
  ...
  const { accountId, groupNo, shortId, content, version, log } = params;
  const meta: GroupMdMeta = {
    version,
    updated_at: new Date().toISOString(),
    updated_by: "event",
  ...
  try {
    ...
    log?.info?.(`octo: broadcastThreadMdUpdate: updated disk cache thread=${groupNo}/${shortId} v=${version}`);
  } catch (err) {
    log?.error?.(`octo: broadcastThreadMdUpdate failed for thread=${groupNo}/${shortId}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
